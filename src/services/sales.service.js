...
const findById = async (saleId) => {
  const saleById = await saleModel.findById(saleId);
  if (!saleById.length) return { type: 'NOT_FOUND', message: 'Sale not found' };

  return { type: null, message: saleById };
};

const deleteSaleById = async (saleId) => {
  const deleteSale = await saleModel.findById(saleId);
  if (!deleteSale.length) return { type: 'NOT_FOUND', message: 'Sale not found' };
